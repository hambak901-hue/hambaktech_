...
export async function GET(req: NextRequest) {
  try {
    ...
    return errorResponse(error, "Failed to retrieve assignments");
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!body.assignmentId || !body.enrollmentId || !body.content) {
      return errorResponse(new Error("assignmentId, enrollmentId, and content are required"), "Missing required fields", 400);
    }
    ...
    return errorResponse(error, "Failed to submit assignment");
  }
}
